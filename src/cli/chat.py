...
def chat(session: Optional[str] = typer.Option(None, help="Resume a previous conversation by session ID.")):
    """Start an authenticated interactive chat session."""
    store = TokenStore()
    if not store.load():
        typer.echo("Not logged in. Run `finance-helper auth login` first.", err=True)
    ...
    if store.is_expired():
        try:
            ...
            typer.echo("Session expired and refresh failed. Run `finance-helper auth login`.", err=True)
            raise typer.Exit(code=1)
...
def _invoke(runtime_arn: str, session_id: str, id_token: str, prompt: str) -> str:
    # The custom_jwt_authorizer replaces SigV4 — the Entra token IS the credential.
...
def _run_chat_loop(config: dict, tokens: dict, session_id: str) -> None:
    while True:
        ...
        try:
            response_text = _invoke(config["runtime_arn"], session_id, tokens["id_token"], prompt)
